function [qrs_amp_raw,qrs_i_raw,delay]=ivan(ecg,fs,gr)
...
if fs ~= 360
    error('fs must be 30Hz');
else
  % ------------------ remove the mean of Signal -----------------------%
  ecg = ecg - mean(ecg);

   %% ==== Low Pass Filter  H(z) = ((1 - z^(-6))^2)/(1 - z^(-1))^2 ==== %%
   Bnf = conv ([1 zeros(1,7) -1], [1 zeros(1, 7) -1]); 
   Anf = conv([1 -1], [1 -1]); 
   %ecg_l = filter(Bnf,Anf,ecg); 
   [Q,R] = deconv(Bnf,Anf);
   ecg_l= filtfilt(Q,1,ecg);
   ...
   %% ==== High Pass filter H(z) = (-1+32z^(-16)+z^(-32))/(1+z^(-1)) ==== %%
    Bvf = [1 zeros(1, 15) -1];
    Avf = [1 1]; % DA LI MINUS JEDAN
    %ecg_h = filter(Bvf,Avf,ecg_l);
   [Q,R] = deconv(Bvf,Avf);
   ecg_h= filtfilt(Q,1,ecg_l);
...
%% ===================== Fiducial Marks ============================== %% 
% Note : a minimum distance of 72 samples is considered between each R wave
